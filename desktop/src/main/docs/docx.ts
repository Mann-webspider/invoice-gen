/* eslint-disable */
// @ts-nocheck
// Copied from ui/src/lib/wordGenerator.ts. Only the entry signature changed:
// the browser version returned a Blob for file-saver and was exported under the
// name `generateInvoigenerateDocxceExcel`.

import type { LegacyInvoiceData } from './excel/types'
import { Document, Packer, Paragraph, TextRun } from "docx";

export const generateDocx = async (data: LegacyInvoiceData): Promise<Buffer> => {
  ...
  // Packer.toBuffer, not toBlob: this runs in Node now, and the caller
  // writes the result straight to disk.
