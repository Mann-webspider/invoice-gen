...
export const generateInvoigenerateDocxceExcel = async (data): Promise<void> => {
    ...
    let notifyAndConsignee = formattedNotifyParty + "\n" + formattedConsignee;
        ...
        sections: [
            {
                children: [
                    ...
                    (() => {
                        const notifyLines = notifyParty.split("\n").map(line => line.trim()).filter(Boolean);

                        function isName(line) {
                            // A name contains only alphabets and spaces (no digits or special chars)
                        ...
                        for (const line of notifyLines) {
                            if (isName(line)) {
                                if (currentGroup.length > 0) {
                                    groupedNotifies.push(currentGroup);
                                    currentGroup = [];
                                }
                            }
