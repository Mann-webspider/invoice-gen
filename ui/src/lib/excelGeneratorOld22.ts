...
let isRemarks = data?.is_remark === 1? true: false || false;
    ...
    if (type !== "tiles") {
        ...
        if (isRemarks) {
            ...
            worksheet.getCell('E' + (row - 2)).alignment = { vertical: 'top', wrapText: true };

            // Adjust height automatically
            let lineCount = remarks.split(/\r?\n/).length;
            if (lineCount < 1) { lineCount = 1; }
            worksheet.getRow(row - 1).height = 15 * lineCount;
    ...
    if (termsOfDeliveryMain !== "FOB") {
        ...
        if (type !== "tiles") {
            for (let i = 0; i < skipRow; i++) {
                if (i == skipRow - 1 && isRemarks) {
                    worksheetCopy.mergeCells('S' + (row - 1) + ':T' + (row - 1)); worksheetCopy.mergeCells('U' + (row - 1) + ':V' + (row - 1)); worksheetCopy.mergeCells('W' + (row - 1) + ':X' + (row - 1));
                } else {
            ...
            if (isRemarks) {
                ...
                // worksheet.getCell('E' + (row - 2)).font = { bold: true };
                worksheetCopy.getCell('E' + (row - 2)).alignment = { vertical: 'top', wrapText: true };

                // Adjust height automatically
                let lineCount = remarks.split(/\r?\n/).length;
                if (lineCount < 1) { lineCount = 1; }
                worksheetCopy.getRow(row - 1).height = 15 * lineCount;
            ...
            for (let i = 0; i < skipRow; i++) {
                if (i == skipRow - 1 && isRemarks) {
                    worksheetCopy.mergeCells('R' + (row - 1) + ':S' + (row - 1)); worksheetCopy.mergeCells('T' + (row - 1) + ':U' + (row - 1)); worksheetCopy.mergeCells('V' + (row - 1) + ':X' + (row - 1)); worksheetCopy.mergeCells('Y' + (row - 1) + ':AA' + (row - 1));
                } else {
                    setOuterBorder('E' + row + ':Q' + row, worksheetCopy);
                    setOuterBorder('R' + row + ':S' + row, worksheetCopy);
                    setOuterBorder('T' + row + ':U' + row, worksheetCopy);
